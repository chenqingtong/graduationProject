...
public class WfDeployServiceImpl extends FlowServiceFactory implements IWfDeployService {
    ...
    public void updateApprovalSet2(ApprovalSetDTO approvalSetDTO, String type) {
        ...
        // 如果审批设置不存在，创建新的记录
        if (materialsApprovalSet == null) {
            materialsApprovalSet = new MaterialsApprovalSet();
            materialsApprovalSet.setExtraId(approvalSetDTO.getTaskId());
        ...
        if ("0".equals(approvalSetDTO.getApproved())) {
            ...
            if (StringUtils.isNotBlank(approvalSetDTO.getApprovalInfo())) {
                materialsApprovalSet.setApprovalInfo(approvalSetDTO.getApprovalInfo());
            } else {
                // 如果需要审批但没有提供审批人信息，清空（避免使用旧值）
                materialsApprovalSet.setApprovalInfo(null);
            }
        } else {
            // 无需审批：清空审批人信息
            materialsApprovalSet.setApprovalInfo(null);
        ...
        // 根据是否存在来决定是插入还是更新
        if (materialsApprovalSet.getId() == null) {
            materialsApprovalSetMapper.insert(materialsApprovalSet);
        } else {
