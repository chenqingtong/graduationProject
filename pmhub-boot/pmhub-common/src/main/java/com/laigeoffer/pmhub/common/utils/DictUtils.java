...
import com.laigeoffer.pmhub.common.core.domain.entity.SysDictData;
    ...
    public static List<SysDictData> getDictCache(String key) {
        ...
        if (StringUtils.isNotNull(arrayCache)) {
            try {
                ...
                // 缓存中可能包含旧的类路径数据，清除缓存并返回null，让系统重新从数据库加载
                log.warn("字典缓存反序列化失败，清除缓存: key={}, error={}", key, e.getMessage());
                removeDictCache(key);
                return null;
            }
        }
