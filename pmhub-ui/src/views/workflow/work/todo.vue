...
export default {
  ...
  methods: {
    ...
    // 跳转到处理页面
    handleProcess(row) {
      console.log('handleProcess row:', row);
      console.log('procDefId:', row.procDefId);
      console.log('procVars:', row.procVars);
      console.log('taskId:', row.taskId);
      console.log('procInsId:', row.procInsId);
      
      // 判断是否为简化审批任务（简化审批任务的 procDefId 以 "simplified-approval-" 开头）
      if (row.procDefId && row.procDefId.startsWith('simplified-approval-')) {
        // 简化审批任务：跳转到简化审批页面进行审批
        // procVars 可能是 Object 类型，需要兼容不同的访问方式
        let procVars = row.procVars;
        let url = null;
        let type = null;
        let extraId = null;
        
        if (procVars && typeof procVars === 'object') {
          // 获取URL，兼容 .url 和 ['url'] 两种访问方式
          url = procVars.url || procVars['url'];
          type = procVars.type || procVars['type'];
          extraId = procVars.extraId || procVars['extraId'];
        }
        
        // 从 row 对象中获取任务信息
        let title = row.taskName || row.procDefName || '任务审批';
        let initiatorName = row.startUserName || '';
        
        console.log('简化审批任务 - taskId:', row.taskId, 'url:', url, 'type:', type, 'extraId:', extraId, 'title:', title);
        
        // 跳转到简化审批页面
        // row.taskId 就是审批任务ID（approvalTaskId）
        try {
          const query = {
            approvalTaskId: row.taskId
          }
          ...
          if (initiatorName) {
            query.initiatorName = encodeURIComponent(initiatorName)
          }
          if (url) {
          ...
          this.$router.push({
            path: '/workflow/work/simplified-approval',
            query: query
          }).catch(err => {
            console.error('路由跳转失败:', err);
            this.$modal.msgError('无法跳转到审批页面，可能是权限不足或路由未配置');
          });
          return;
        } catch (err) {
