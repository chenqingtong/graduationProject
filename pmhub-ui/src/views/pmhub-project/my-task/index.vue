<template>
  <div class="app-container" v-loading="loading">
    ...
    <el-dialog
      title="审批进度"
      :visible.sync="approvalProgressDialogVisible"
      width="60%"
      :close-on-click-modal="false"
    >
      <el-table :data="approvalTaskList" border style="width: 100%">
        <el-table-column prop="approverName" label="审批人" width="150" />
        <el-table-column prop="approverType" label="审批人类型" width="120">
          <template slot-scope="scope">
            <span v-if="scope.row.approverType === 'user'">指定用户</span>
            <span v-else-if="scope.row.approverType === 'role'">角色</span>
            <span v-else-if="scope.row.approverType === 'dept'">部门</span>
            <span v-else-if="scope.row.approverType === 'initiator'">发起人</span>
            <span v-else>{{ scope.row.approverType }}</span>
          </template>
        </el-table-column>
...
import { getApprovalTaskList } from "@/api/workflow/process.js"
  ...
  methods: {
    ...
    async handleApproved(row) {
      try {
        ...
        console.log("点击审批按钮，taskId:", row.taskId)
        console.log("workFlowable对象:", this.workFlowable)
        const approved = this.workFlowable?.approved
        console.log("approved值:", approved, "类型:", typeof approved)
        if (approved === undefined || approved === "1") {
          console.log("审批未开启，approved:", approved)
          this.$modal.msgWarning("请先去任务设置页面开启审批并设置审批人")
          return
        }
        // 验证审批人设置是否存在
        const approvalInfo = this.workFlowable?.approvalInfo
        console.log("approvalInfo值:", approvalInfo, "类型:", typeof approvalInfo)
        if (!approvalInfo) {
          console.log("approvalInfo为空")
        ...
        try {
          ...
          console.log("解析后的approvalInfoObj:", approvalInfoObj)
          // 检查是否有有效的审批人信息
          ...
          console.log("hasValidApprover:", hasValidApprover, 
          ...
          console.error("解析approvalInfo失败:", e)
          this.$modal.msgWarning("审批人设置不完整，请先在任务设置页面设置审批人")
          return
        }
        // 所有验证通过后，才打开对话框
        ...
        // 如果获取审批设置失败，显示错误提示
        this.$modal.msgError(error.msg || "获取审批设置失败，请重试")
        // 确保对话框状态为关闭
        this.startDialogVisible = false
      }
    },
    async handleApprovalProgress(row) {
      try {
        // 调试日志
        console.log("审批进度点击，row数据：", row)
        console.log("workFlowable数据：", row.workFlowable)
        
        // 检查 workFlowable 是否存在
        if (!row.workFlowable) {
          this.$modal.msgWarning("无法获取审批信息，请刷新页面后重试")
          return
        }
        
        const approved = row.workFlowable?.approved
        const procInsId = row.workFlowable?.procInsId
        const taskProcessId = row.workFlowable?.taskProcessId
        
        console.log("审批状态：", approved, "流程实例ID：", procInsId)
        
        // 检查是否已发起审批
        // approved 字段含义：
        // - null/""/undefined: 未发起审批
        // - "0": 审批中
        // - "1": 已通过
        // - "2": 已拒绝
        // 只有当 approved 为空、null 或 undefined 时，才提示"请先发起审批"
        if (approved === null || approved === undefined || approved === "") {
          this.$modal.msgWarning("请先发起一个审批流程")
          return
        }
        
        // 如果已发起审批（approved === "0"/"1"/"2"），可以查看审批进度
        // 如果有流程实例ID，跳转到流程详情页（Flowable流程）
        if (procInsId && procInsId !== "" && procInsId !== null) {
          this.$router.push({
          ...
          // 如果没有流程实例ID，说明是简化审批流程
          // 查询审批任务列表并显示在对话框中
          this.currentTaskId = row.taskId
          try {
            const res = await getApprovalTaskList(row.taskId, "task")
            this.approvalTaskList = res.data || []
            this.approvalProgressDialogVisible = true
          } catch (error) {
            console.error("查询审批任务列表失败：", error)
            this.$modal.msgError("查询审批进度失败，请刷新页面后重试")
          }
        }
      } catch (error) {
        console.error("审批进度查看出错：", error)
        this.$modal.msgError("查看审批进度时出错，请刷新页面后重试")
      }
    },
